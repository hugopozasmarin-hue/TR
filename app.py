...
with tab1:
    if st.button(t["btn"]):
        with st.status(t["wait"]):
            ...
            if not data.empty:
                # --- CORRECCIÓN COLUMNAS YFAT ---
                if data.columns.nlevels > 1: data.columns = data.columns.droplevel(-1)
                ...
                else: st.error("Error en el modelo de predicción.")
