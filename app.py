...
def generar_analisis_ia(ticket, p_act, p_fut, cambio, perfil, capital, pregunta=None):
    try:
        ...
        prompt = f"""
Actúa como analista senior de Wall Street, experto en inversiones.
Activo: {ticket}.
Perfil de riesgo: {perfil}.
Capital disponible: {capital}€.
Precio actual: {p_act}€.
Predicción IA a 30 días: {p_fut}€ ({cambio:.2f}%).
Tarea: Analiza riesgos, fiabilidad de la tendencia y da una recomendación clara.
Pregunta del usuario: {pregunta if pregunta else "Análisis general de inversiones"}.
"""
...
with tab2:
    st.markdown("### 💬 Chat abierto de inversión")
