...
# 4. BARRA LATERAL (solo ajustes, sin “configuración”)
with st.sidebar:
    ...
    if st.button(t["btn"]):
        with st.status(t["wait"]) as s:
            ...
            if not datos.empty:
                ...
                # --- MÉTRICAS ENCIMA DE LA GRÁFICA ---
                col1, col2, col3 = st.columns(3)
                simbolo = "€"  # siempre euros

                col1.metric(label=f"💰 {t['price']}", value=f"{st.session_state.p_act:.2f}{simbolo}")
                col2.metric(label=f"📈 {t['target']}", value=f"{st.session_state.p_pre:.2f}{simbolo}", delta=f"{st.session_state.cambio:.2f}%")
                col3.metric(label=f"🧮 {t['shares']}", value=f"{(capital/st.session_state.p_act):.4f}")
