...
def generar_analisis_ia(lang, ticket, p_act, p_fut, perfil, capital, pregunta=None):
    try:
        ...
        contexto_mercado = f"Activo actual: {ticket}. Precio: {p_act}€. Predicción: {p_fut}€." if ticket else "Sin activo seleccionado."

        prompt = f"""
        Actúa como un Estratega Senior de Inversiones. Responde en {idioma_inst}.
        Contexto: Perfil {perfil}, Capital {capital}€. {contexto_mercado}.
        Instrucción: Responde cualquier duda sobre inversiones o finanzas de forma profesional.
...
with tab1:
    if st.button(t["btn"]):
        with st.spinner(t["wait"]):
            ...
            if not data.empty:
                # CORRECCIÓN PARA EL ERROR DE LA IMAGEN: Aplanar MultiIndex
                if isinstance(data.columns, pd.MultiIndex):
                    data.columns = data.columns.get_level_values(0)
                
                # Asegurar que 'y' sea una serie 1D limpia
                df = data[['Close']].copy()
                df = df.reset_index()
                df.columns = ['ds', 'y']
